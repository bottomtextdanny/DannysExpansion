...
public class IceElementalPsyche extends Psyche<IceElemental> {
    ...
    public static final MobPosProcessor<Object> STROLL = compose(
            (pos, mob, r, extra) -> {
                FloatMapper view = DEFloatMappers.range(mob.getYHeadRot() * BCMath.FRAD, 40.0F * BCMath.FRAD, FloatRandomPicker.normal());
                ...
                return sample(10, stack()
                                .push(advanceHorizontal(view, FloatMapper.from(2, 4, FloatRandomPicker.linear())))
                                .push(randomOffset(5, 5, 5)),
                        evaluatePos(mob)
                ).then(find(10, stack()
                        .push(randomOffset(4, 3, 4))
                        .push(pred(stable.negate()))
                        .push(testNull(stack()
    ...
    private MoveReactionToEntityAction<IceElemental, LivingEntity> flyAwayFromLookAction;
    private MoveReactionToEntityAction<IceElemental, LivingEntity> flyToLookAction;
    ...
    protected void initialize() {
        ...
        this.flyAwayFromLookAction = new MoveReactionToEntityAction<>(getMob(), TargetRange.followRangeMultiplied(IceElemental.FLEE_RANGE).cast(), AVOID_POS)
                .speedByTarget(target -> IceElemental.COMBAT_MOVE_SPEED)
                .setRefreshRate(15)
                .searchRange(() -> 0.0F)
    ...
    public static class IceSpikeAttackAction extends AnimationAction<IceElemental, SimpleAnimation> {
        ...
        public void update() {
            ...
            if (this.animationHandler.getTick() == 4) {
                ...
                IceSpike iceSpike = new IceSpike(DEEntities.ICE_SPIKE.get(), this.mob.level);
