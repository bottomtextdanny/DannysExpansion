...
public final class DERayUtil {
    ...
    public static EntityHitResult rayTraceEntitiesBullet(Level worldIn, Entity projectile, Vec3 startVec, Vec3 endVec, AABB boundingBox, Predicate<Entity> filter) {
        ...
        for(Entity entity1 : worldIn.getEntities(projectile, boundingBox, filter)) {
            ...
            float hlp = 0.2F;
            float w = entity1.getBbWidth() / 2 + hlp;
            float h = entity1.getBbHeight() + hlp;
            ...
            for (int i = 0; i < 4; i++) {
                ...
                AABB axisalignedbb = new AABB(easedVec.add(-w, -h, -w), easedPosition.add(w, h, w));
                Optional<Vec3> optional = axisalignedbb.clip(startVec, endVec);
    ...
    public static HitResult orbRaytrace(Entity entity, Predicate<Entity> predicate, Vec3 vec, ClipContext.Fluid fluidMode, float inflation) {
        ...
        EntityHitResult entityraytraceresult = rayTraceEntitiesOrb(entity.level, entity, vector3d2, vector3d3, entity.getBoundingBox().expandTowards(vec).inflate(inflation), predicate);
        if (entityraytraceresult != null) {
            raytraceresult = entityraytraceresult;
        }
    ...
    public static boolean noCollision(Level level, Vec3 vector1, Vec3 vector2) {
        if (vector1.distanceTo(vector2) > 128.0D) {
            return false;
        } else {
            return level.clip(new ClipContext(vector1, vector2, ClipContext.Block.COLLIDER, ClipContext.Fluid.NONE, null)).getType() == HitResult.Type.MISS;
        }
    }
