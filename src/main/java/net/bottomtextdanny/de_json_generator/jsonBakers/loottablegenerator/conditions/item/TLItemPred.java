...
public class TLItemPred extends JsonDecoder {
    ...
    public TLItemPred bake() {
        ...
        if (this.minimumCount.isPresent()) {
            ...
            countObj.add("min", cInt(this.minimumCount.get()));
            countObj.add("max", cInt(this.maximumCount.get()));
            this.jsonObj.add("count", countObj);
        }

        if (this.minimumDurability.isPresent()) {
            JsonObject countObj = new JsonObject();
            countObj.add("min", cInt(this.maximumDurability.get()));
            countObj.add("max", cInt(this.maximumDurability.get()));
