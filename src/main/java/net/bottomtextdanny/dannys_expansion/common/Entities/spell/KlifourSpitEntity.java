...
public class KlifourSpitEntity extends SpellEntity {
    ...
    public void tick() {
        ...
        if (this.level.isClientSide) {
            ...
            if (this.random.nextInt(2) == 1) this.level.addParticle(DEParticles.KLIFOUR_POISON_BUBBLE.get(), position().x + xRand0, position().y + 0.125F + yRand0, position().z + zRand0,0.0D, 0.0D, 0.0D);
            ...
            if (this.random.nextInt(2) == 1) this.level.addParticle(DEParticles.KLIFOUR_POISON_DRIP.get(), position().x + xRand1, position().y + 0.125F + yRand1, position().z + zRand1,0.0D, 0.0D, 0.0D);
    ...
    protected void onEntityHit(EntityHitResult result) {
        ...
        if (!(result.getEntity() instanceof KlifourEntity)) {
    ...
    public HitResult rayTraceResultType() {
        return DannyRayTraceHelper.orbRaytrace(this, this::collisionParameters, getDeltaMovement().add(this.externalMotion.getAcceleratedMotion()), ClipContext.Fluid.ANY);
    }
