...
public interface IExtraModelProvider extends IForgeItem, ExtraItemModelLoader {
    Map<Item, Int2ObjectArrayMap<ModelResourceLocation>> MODEL_FETCH = new HashMap<>();

    default void fetchFrom(Pair<Integer, ModelResourceLocation>... models) {
        Connection.doClientSide(() -> {
            Int2ObjectArrayMap<ModelResourceLocation> int2ModelMap = new Int2ObjectArrayMap<>(models.length);

            for (Pair<Integer, ModelResourceLocation> model : models) {
                int2ModelMap.put(model.getLeft(), model.getRight());
            }
